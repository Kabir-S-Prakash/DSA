...
bool SC(graph g){
    // Reading graphs
    map<int,vector<int> > adj1 = g.adj;
    int n = adj1.size();
    // Reverse graph
    graph G = reverseEdges(g);
    map<int,vector<int> > adj2 = G.adj;
    ...
    // DFS of arbitrary
    int time = 0;
    dfs(0,arrival,departure,adj1,time);
    bool first = true;
    for(int i=0; i<n; i++){
        if(arrival[i] == -1) first = false;
    }
    if(!first) return false;

        // Resetting arrival,departure and time
    ...
    time = 0;
    dfs(0,arrival,departure,adj2,time);
    bool second = true;
    for(int i=0; i<n; i++){
        if(arrival[i] == -1) second = false;
    }
